...
single_count=$(wc -l < /tmp/mb_api_one_out.txt)
batch_count=$(wc -l < /tmp/mb_api_batch_out.txt)
...
rc=0; "$API_DIR/mbmap-one" /tmp/nonexistent_index /tmp/mb_test_noop.fa 2>/dev/null || rc=$?
if [ "$rc" -ne 0 ]; then
